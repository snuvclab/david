...
from imports.genpose.utils.genpose_utils import get_pose_dim, get_pose_dim_RT_sequence, get_pose_dim_RT_sequence_flexible
from imports.genpose.networks.decoder_head.rot_head import RotHead
from imports.genpose.networks.decoder_head.trans_head import TransHead
from k import k
...
class PoseScoreNet(nn.Module):
    def __init__(self, marginal_prob_func, pose_mode='quat_wxyz', regression_head='RT', per_point_feature=False):
        ...
        pose_dim = get_pose_dim_RT_sequence_flexible(pose_mode)

        if regression_head == 'Rx_Ry_T_S_both':
            pose_dim = 24
        ''' encode pose '''
        ...
        if self.regression_head == 'RT':
            ...
            if per_point_feature:
                ...
                self.fusion_tail_rot_x = nn.Sequential(
                    nn.Linear(128+256+512, 256),
                    # nn.BatchNorm1d(256, eps=1e-05, momentum=0.1, affine=True, track_running_stats=True), 
                    self.act,
                    nn.Linear(256, 256),
                    self.act,
                    zero_module(nn.Linear(256, 3 * k)),
                )
        ...
        elif self.regression_head == 'Rx_Ry_T_custom':
            if pose_mode != 'rot_matrix':
                raise NotImplementedError
            if per_point_feature:
                self.fusion_tail_rot_x = RotHead(in_feat_dim=128+256+1280, out_dim=3)       # t_feat + pose_feat + pts_feat
                ...
                self.fusion_tail_target_rot_x = nn.Sequential(
                    #nn.Linear(128+256+1024, 256),
                    nn.Linear(128+256, 256),
                    # nn.BatchNorm1d(256, eps=1e-05, momentum=0.1, affine=True, track_running_stats=True), 
                    self.act,
                ...
                self.fusion_tail_target_rot_y = nn.Sequential(
                    #nn.Linear(128+256+1024, 256),
                    nn.Linear(128+256, 256),
                    # nn.BatchNorm1d(256, eps=1e-05, momentum=0.1, affine=True, track_running_stats=True), 
                    self.act,
                ...
                self.fusion_tail_target_trans = nn.Sequential(
                    #nn.Linear(128+256+1024, 256),
                    nn.Linear(128+256, 256),
                    # nn.BatchNorm1d(256, eps=1e-05, momentum=0.1, affine=True, track_running_stats=True), 
                    self.act,
                ...
                self.fusion_tail_target_scale = nn.Sequential(
                    #nn.Linear(128+256+1024, 256),
                    nn.Linear(128+256, 256),
                    # nn.BatchNorm1d(256, eps=1e-05, momentum=0.1, affine=True, track_running_stats=True), 
                    self.act,
    ...
    def forward(self, data):
        ...
        total_feat = torch.cat([theta_feat, t_feat, pose_feat], dim=-1)
        ...
        if self.regression_head == 'RT':
            ...
            rot_x = self.fusion_tail_rot_x(total_feat) # B x 360
            rot_y = self.fusion_tail_rot_y(total_feat) # B x 360
            trans = self.fusion_tail_trans(total_feat) # B x 360

            # out_score = torch.cat([rot_x, rot_y, trans], dim=-1) / (std+1e-7) # normalisation

            rot_x_ = rot_x.unsqueeze(2)
            rot_y_ = rot_y.unsqueeze(2)
            interleaved = torch.cat([rot_x_, rot_y_], dim=2)
            interleaved_concat = interleaved.reshape(rot_x.shape[0], -1) # B x 720
            out_score = torch.cat([interleaved_concat, trans], dim=-1) / (std+1e-7) # normalisation
        else:
            raise NotImplementedError
        # set_trace()
        return out_score
