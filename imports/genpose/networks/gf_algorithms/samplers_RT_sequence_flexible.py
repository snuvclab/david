...
def cond_ode_sampler_for_RT_inference(
    ...
    def contact_loss(x):
        ...
        # obj_R = get_rot_matrix(x[:, :6], 'rot_matrix') # frame_num x 3 x 3
        obj_R = get_rot_matrix(x[:, :6 * k].reshape((-1, 6)), 'rot_matrix') # frame_num x 3 x 3
        if ratio is None:
            obj_T = x[:, 6 * k:9 * k].reshape((-1, 3)).unsqueeze(2) # frame_num x 3 x 1
        else: obj_T = x[:, 6 * k:9 * k].reshape((-1, 3)).unsqueeze(2) * ratio # frame_num x 3 x 1
        ...
        return minimum_distance, contact_loss * 15
    ...
    # sol = odeint(ode_func, x, t_span, rtol=rtol, atol=atol, method='rk4')  # or 'dopri5'
