...
from imports.genpose.networks.gf_algorithms.samplers_RT_sequence_flexible import cond_ode_likelihood, cond_ode_sampler, cond_pc_sampler, cond_ode_sampler_for_RT, cond_ode_sampler_for_RT_inference
# from imports.genpose.networks.gf_algorithms.samplers import cond_ode_likelihood, cond_pc_sampler, cond_ode_sampler_for_RT
from imports.genpose.networks.gf_algorithms.scorenet_RT_sequence_flexible import PoseScoreNet, PoseDecoderNet
...
class GFObjectPose(nn.Module):
    def __init__(self, cfg, prior_fn, marginal_prob_fn, sde_fn, sampling_eps, T):
        ...
        ''' encode theta '''
        self.theta_encoder = ThetaEncoder(theta_dim=k * 21 * 3, out_dim=512) # SMPLX theta dimension
    ...
    def extract_theta_feature(self, data):
        thetas = data['thetas'] # B x 120 x 21 x 3
        print(f"thetas: {thetas.shape}")
        theta_feat = self.theta_encoder(thetas.reshape((thetas.shape[0], -1))) # B x (120 x 21 x 3)
        return theta_feat
