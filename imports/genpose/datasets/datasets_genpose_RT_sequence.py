...
class RTDataSet(data.Dataset):
    def __init__(self, data_dir=None):
        ...
        with open(self.data_file_path, 'rb') as f:
            ...
            self.transform_data = pickle_data['transform'] # N x 120 x 3 x 4
            self.point_data = pickle_data['points'] # N x 120 x 10475 x 3
            self.body_pose_data = pickle_data['poses'] # N x 120 x 21 x 3
    ...
    def __getitem__(self, index):
        ...
        body_pose = self.body_pose_data[index, :, :, :3] # 120 x 21 x 3
        base_rotation = self.transform_data[index, :, :, :3] # 120 x 3 x 3
        base_translation = self.transform_data[index, :, :, 3] # 120 x 3 x 1
        ...
        data_dict['thetas'] = torch.as_tensor(body_pose, dtype=torch.float32).contiguous() # 120 x 3 x 1
        data_dict['base_rotation'] = torch.as_tensor(base_rotation, dtype=torch.float32).contiguous() # 120 x 3 x 3
        data_dict['base_translation'] = torch.as_tensor(base_translation, dtype=torch.float32).contiguous() # 120 x 3 x 1
...
def process_batch(batch_sample,
    ...
    thetas_da = batch_sample['thetas'].to(device) # B x 120 x 21 x 3
    gt_base_R_da = batch_sample['base_rotation'].to(device) # B x 120 x 3 x 3
    gt_base_t_da = batch_sample['base_translation'].to(device) # B x 120 x 3 x 1

    processed_sample = {}
    
    processed_sample['thetas'] = thetas_da                # B x 120 x 21 x 3
    ...
    gt_base_t_da_ = gt_base_t_da.reshape(gt_base_t_da.shape[0], -1) # B x (120 x 3)
