...
class RTDataSet(data.Dataset):
    def __init__(self, data_dir=None):
        ...
        with open(self.data_file_path, 'rb') as f:
            ...
            self.transform_data = pickle_data['transform']
            self.point_data = pickle_data['points']
            self.body_pose_data = pickle_data['poses']
    ...
    def __getitem__(self, index):
        ...
        body_pose = self.body_pose_data[index, :, :3] 
        base_rotation = self.transform_data[index, :, :3]
        base_translation = self.transform_data[index, :, 3]
        ...
        data_dict['thetas'] = torch.as_tensor(body_pose, dtype=torch.float32).contiguous()
        data_dict['base_rotation'] = torch.as_tensor(base_rotation, dtype=torch.float32).contiguous()
        data_dict['base_translation'] = torch.as_tensor(base_translation, dtype=torch.float32).contiguous()
...
def process_batch(batch_sample,
    ...
    thetas_da = batch_sample['thetas'].to(device)
    gt_base_R_da = batch_sample['base_rotation'].to(device)
    gt_base_t_da = batch_sample['base_translation'].to(device)

    processed_sample = {}
    processed_sample['thetas'] = thetas_da                # [bs, 22, 3]
    ...
    if pose_mode == 'quat_xyzw':
        ...
        base_rot = pytorch3d.transforms.matrix_to_rotation_6d(gt_base_R_da.permute(0, 2, 1)).reshape(gt_base_R_da.shape[0], -1)
    else:
        raise NotImplementedError
    
    base_location = gt_base_t_da # [bs, 3]
    
    processed_sample['gt_pose'] = torch.cat([base_rot.float(), base_location.float()], dim=-1) # [bs, 6 + 3]  #### [bs, 4/6/3 + 3]
