...
from imports.genpose.utils.data_augmentation import defor_2D, get_rotation
from imports.genpose.utils.data_augmentation import data_augment
from imports.genpose.utils.datasets_utils import aug_bbox_DZI, get_2d_coord_np, crop_resize_by_warp_affine
from imports.genpose.utils.sgpa_utils import load_depth, get_bbox
from imports.genpose.configs.config import get_config
from imports.genpose.utils.misc import get_rot_matrix
from k import k
...
class RTDataSet(data.Dataset):
    def __init__(self, data_dir=None):
        ...
        self.data_file_path = os.path.join(self.data_dir, "RT.pkl")
        with open(self.data_file_path, 'rb') as f:
            pickle_data = pickle.load(f)

            # k = 2
            data_num = pickle_data['transform'].shape[0]
            frames = pickle_data['transform'].shape[1]

            self.transform_data = pickle_data['transform'].reshape((int(data_num * frames / k), k, 3, 4)) # N x k x 3 x 4
            self.point_data = pickle_data['points'].reshape((int(data_num * frames / k), k, 10475, 3)) # N x k x 10475 x 3
            self.body_pose_data = pickle_data['poses'].reshape((int(data_num * frames / k), k, 21, 3)) # N x k x 21 x 3

            print(f"body_pose_data: {pickle_data['poses'].shape}")
            self.length = self.transform_data.shape[0]
        
        # self.normalize_data = normalize_data
        # if normalize_data:
        #     self.mean_std_file_path = os.path.join(self.data_dir, "mean_std_teapot_teacup.pkl")
        #     with open(self.mean_std_file_path, 'rb') as f:
    ...
    def __getitem__(self, index):
        # pts = self.point_data[index, :, :3] 
        body_pose = self.body_pose_data[index, :, :, :3] # k x 21 x 3
        base_rotation = self.transform_data[index, :, :, :3] # k x 3 x 3
        base_translation = self.transform_data[index, :, :, 3] # k x 3 x 1
        # base_scale = self.transform_data[index, :, 4]
        
        # target_rotation = self.transform_data[index, :, 5:8]
        # target_translation = self.transform_data[index, :, 8]
        # target_scale = self.transform_data[index, :, 9]

        data_dict = {}
        # data_dict['pts'] = torch.as_tensor(pts, dtype=torch.float32).contiguous()
        data_dict['thetas'] = torch.as_tensor(body_pose, dtype=torch.float32).contiguous() # k x 3 x 1
        data_dict['base_rotation'] = torch.as_tensor(base_rotation, dtype=torch.float32).contiguous() # k x 3 x 3
        data_dict['base_translation'] = torch.as_tensor(base_translation, dtype=torch.float32).contiguous() # k x 3 x 1

        # data_dict['base_scale'] = torch.as_tensor(base_scale, dtype=torch.float32).contiguous()
        # data_dict['target_rotation'] = torch.as_tensor(target_rotation, dtype=torch.float32).contiguous()
        # data_dict['target_translation'] = torch.as_tensor(target_translation, dtype=torch.float32).contiguous()
        # data_dict['target_scale'] = torch.as_tensor(target_scale, dtype=torch.float32).contiguous()
        # if self.normalize_data:
        #     data_dict['mean_data'] = self.mean_std['mean']
        #     data_dict['std_data'] = self.mean_std['std']
...
def get_data_loaders(
    ...
    dataset = RTDataSet(data_dir=data_path)
    # dataset = NOCSDataSet(
...
def process_batch(batch_sample,
    ...
    thetas_da = batch_sample['thetas'].to(device) # B * frame / k x k x 21 x 3
    gt_base_R_da = batch_sample['base_rotation'].to(device) # B * frame / k x k x 3 x 3
    gt_base_t_da = batch_sample['base_translation'].to(device) # B * frame / k x k x 3 x 1

    processed_sample = {}
    processed_sample['thetas'] = thetas_da                # B * frame / k x k x 21 x 3
    if pose_mode == 'quat_xyzw':
        ...
        batch_size = gt_base_R_da.shape[0]
        gt_base_R_da_ = gt_base_R_da.reshape(gt_base_R_da.shape[0] * gt_base_R_da.shape[1], gt_base_R_da.shape[2], gt_base_R_da.shape[3]) # (B x 120) x 3 x 3
        base_rot_ = pytorch3d.transforms.matrix_to_rotation_6d(gt_base_R_da_.permute(0, 2, 1)).reshape(gt_base_R_da_.shape[0], -1) # orig: (B x 120) x 6
        base_rot = base_rot_.reshape(batch_size, -1) # B x (120 x 6)
    
    else:
        raise NotImplementedError
    
    gt_base_t_da_ = gt_base_t_da.reshape(gt_base_t_da.shape[0], -1) # B x (k x 3)
    base_location = gt_base_t_da_ # [bs, 3]

    processed_sample['gt_pose'] = torch.cat([base_rot.float(), base_location.float()], dim=-1) # B x 1080
